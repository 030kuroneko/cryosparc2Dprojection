...
After the winner is selected, regenerate its Matched Projection on the native
Class Average grid, filter that projection and the native Class Average with
the same physical Butterworth band-pass, and calculate a soft-circular,
...
Nyquist metadata. The raw search score remains tied to the bounded Search
Projection and is labelled accordingly.
